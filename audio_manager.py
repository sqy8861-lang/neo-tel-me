...
class AudioManager:
    ...
    def __init__(
        self, sample_rate: int = 16000, chunk: int = 1024, vad_threshold: int = 800
    ):
        """
        初始化音频管理器
    ...
    def start_recording(
        self, audio_callback: Callable, vad_callback: Optional[Callable] = None
    ):
        """
        开始音频采集
            ...
            try:
                ...
                if self.audio_callback:
                    asyncio.run_coroutine_threadsafe(
                        self.audio_callback(data), self.loop
                    )
