...
class CallMessageStorage:
    ...
    def __init__(self, stream_manager: "StreamManager", llm_config: "LLMConfig | None" = None) -> None:
        """
        初始化存储器
    ...
    async def _write_memory_async(self, message: "Message") -> None:
        ...
        try:
            ...
            if result and result.get("mode"):
                logger.info(f"连麦消息记忆写入成功: {message.message_id}, mode: {result['mode']}")
        except Exception as e:
...
- 如果消息包含上述任意一种有价值的信息，返回 should_write: true"""
            ...
            request.add_payload(LLMPayload(ROLE.SYSTEM, Text("你是一个连麦消息记忆判断助手。只返回 JSON，不要其他文本。")))
