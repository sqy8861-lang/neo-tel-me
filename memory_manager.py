...
class MemoryManager:
    ...
    async def fetch_messages_by_stream_id(
        ...
        try:
            ...
            for msg in messages:
                ...
                if content.strip():
                    result.append(
                        {"role": role, "content": content.strip(), "time": msg.time}
                    )
    ...
    async def generate_memory_prompt(
        self, llm_client=None, stream_id: str = "", user_nickname: str = ""
    ) -> str:
        """
        生成记忆提示词
...
3. 最多200字"""
        ...
        conversation_summary, user_memories_summary = await asyncio.gather(
            refine_conversation(), refine_user_memories()
        )
