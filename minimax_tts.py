...
class MiniMaxTTS:
    ...
    async def tts_stream(
        self, text, speed=1, volume=1, pitch=0, sample_rate=32000, format="mp3"
    ):
        """
        生成TTS音频流（使用连接复用）
    ...
    async def _start_task(self, speed, volume, pitch, sample_rate, format):
        ...
        try:
            ...
            print(
                f"发送TTS任务开始请求: model={self.model}, voice_id={self.voice_id}, sample_rate={sample_rate}, format={format}"
            )
