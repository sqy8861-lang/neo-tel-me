...
class NeoTelMeService(BaseService):
    ...
    version: str = "1.1.0"
    update_info: str = (
        "更新：集成阿里云智能语音交互Python SDK，实现Token自动获取和实时语音识别功能"
    )
    """
    Neo-tel-me 服务
    """
    ...
    async def start(
        self, user_nickname: str = "", user_id: str = "", person_id: str = ""
    ):
        """
        启动服务
    ...
    async def _initialize_llm(self):
        ...
        try:
            ...
            if self.system_prompt:
                ...
                personality_prompt = await self.prompt_refiner.initialize(
                    self.llm_client, user_nickname=self.user_nickname
                )
            ...
            self.history_manager = HistoryManager(
                max_history=cfg.llm.prompt.max_history
            )
    ...
    async def _load_or_generate_memory_prompt(self, stream_id: str = "") -> str:
        ...
        memory_prompt = await self.memory_manager.generate_memory_prompt(
            self.llm_client, stream_id=stream_id, user_nickname=self.user_nickname
        )
        ...
        try:
            with open(MEMORY_PROMPT_FILE, "w", encoding="utf-8") as f:
                json.dump(
                    {"memory_prompt": memory_prompt}, f, ensure_ascii=False, indent=2
                )
    ...
    async def _start_local_mode(self):
        ...
        try:
            ...
            self.audio_manager.start_recording(
                audio_callback=self._on_audio_data, vad_callback=self._on_vad
            )
    ...
    async def _start_websocket_mode(self):
        ...
        try:
            ...
            self.websocket_handler.on_client_connected = (
                self._on_websocket_client_connected
            )
            self.websocket_handler.on_client_disconnected = (
                self._on_websocket_client_disconnected
            )
            ...
            logger.info(
                f"WebSocket模式已启动，监听 {self._cfg().websocket.host}:{self._cfg().websocket.port}"
            )
    ...
    def _on_vad(self, volume: float):
        ...
        if (
            ...
            and volume > self._cfg().audio.vad_threshold
        ):
            self.tts.interrupt()
            logger.info("用户打断 AI")
    ...
    async def _generate_reply(self, user_input: str) -> str:
        ...
        try:
            # 获取历史记录
            history_text = (
                self.history_manager.format_for_llm()
                if self.history_manager
                else "暂无对话历史"
            )
            ...
            async for audio_data in self.tts.tts_stream(
                ...
                if self.audio_manager:
                    self.audio_manager.play_audio(
                        audio_data, sample_rate=self._cfg().minimax_tts.sample_rate
                    )
        except Exception as e:
    ...
    async def _on_websocket_asr_result(self, websocket, text: str):
        ...
        try:
            ...
            if (
                not hasattr(websocket, "remote_address")
                or websocket.remote_address is None
            ):
                logger.warning("客户端已断开，跳过处理")
                return

            logger.info(f"客户端 {websocket.remote_address} 用户说: {text}")

            if not hasattr(websocket, "history_manager"):
                websocket.history_manager = HistoryManager(
                    max_history=self._cfg().llm.prompt.max_history
                )
    ...
    async def _generate_reply_for_websocket(self, websocket, text: str) -> str:
        ...
        try:
            # 获取历史记录
            history_text = (
                websocket.history_manager.format_for_llm()
                if hasattr(websocket, "history_manager")
                else "暂无对话历史"
            )
    ...
    async def _generate_and_send_tts_to_websocket(self, websocket, text: str):
        ...
        try:
            ...
            if audio_data and self.websocket_handler:
                logger.info("正在发送音频给客户端...")
                await self.websocket_handler.send_audio_to_client(
                    websocket, audio_data, text
                )
                logger.info("音频发送完成")
            elif not audio_data:
                logger.warning("音频数据为空，跳过发送")
            elif not self.websocket_handler:
                logger.warning("websocket_handler 为空，无法发送音频")
