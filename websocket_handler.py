...
class WebSocketHandler:
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info(
                f"🎙️ WebSocket 服务器已启动，监听 {self.config.websocket.host}:{self.config.websocket.port} ({protocol})"
            )
        ...
        try:
            ...
            subject = issuer = x509.Name(
                [
                    ...
                    x509.NameAttribute(
                        NameOID.COMMON_NAME,
                        self.config.websocket.public_ip or self.config.websocket.host,
                    ),
                ]
            )
            ...
            cert = (
                ...
                .add_extension(
                    x509.SubjectAlternativeName(
                        [
                            x509.DNSName(
                                self.config.websocket.public_ip
                                or self.config.websocket.host
                            ),
    ...
    async def process_request(self, connection, request):
        ...
        try:
            ...
            return Response(
                500, "Internal Server Error", headers, b"Internal Server Error"
            )
    ...
    async def handle_client(self, websocket: websockets.WebSocketServerProtocol):
        ...
        self.client_data[websocket] = {"is_connected": False, "session_id": None}
    ...
    async def handle_message(
        ...
        try:
            ...
            await websocket.send(
                json.dumps({"type": "error", "message": f"处理消息错误: {str(e)}"})
            )

    async def start_call(
        self, websocket: websockets.WebSocketServerProtocol, client_info: Dict
    ):
        """
        开始连麦
        ...
        try:
            ...
            await websocket.send(
                json.dumps(
                    {"type": "status", "status": "connected", "message": "连麦中..."}
                )
            )
            ...
            await websocket.send(
                json.dumps({"type": "error", "message": f"开始连麦错误: {str(e)}"})
            )
    ...
    async def stop_call(
        ...
        try:
            ...
            await websocket.send(
                json.dumps(
                    {"type": "status", "status": "ready", "message": "连麦已结束"}
                )
            )
            ...
            await websocket.send(
                json.dumps({"type": "error", "message": f"结束连麦错误: {str(e)}"})
            )
    ...
    async def handle_audio_data(
        ...
        try:
            ...
            await websocket.send(
                json.dumps({"type": "error", "message": f"处理音频数据错误: {str(e)}"})
            )
    ...
    async def send_audio_to_client(
        ...
        try:
            logger.info(
                f"send_audio_to_client: 音频大小={len(audio_data)}字节, 文本长度={len(text)}"
            )
            ...
            message = json.dumps(
                {"type": "ai_message", "content": text, "audio": audio_base64}
            )
    ...
    async def send_message_to_client(
        ...
        """
        try:
            await websocket.send(json.dumps({"type": message_type, "content": content}))
        except Exception as e:
